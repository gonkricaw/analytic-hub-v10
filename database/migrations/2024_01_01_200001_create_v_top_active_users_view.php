...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if ($driver === 'sqlite') {
            // SQLite-compatible version with simplified date functions
            DB::statement("
                ...
                SELECT 
                    ...
                    COUNT(CASE 
                        ...
                        AND la.attempted_at >= date('now', 'start of month')
                        THEN 1 
                    ...
                    COUNT(CASE 
                        WHEN la.status = 'success' 
                        AND la.attempted_at >= date('now', 'start of month', '-1 month')
                        AND la.attempted_at < date('now', 'start of month')
                        THEN 1 
                    ...
                    ROUND(
                        CAST(COUNT(CASE 
                            WHEN la.status = 'success' 
                            AND la.attempted_at >= date('now', '-6 months')
                            THEN 1 
                        END) AS REAL) / 6.0, 2
                    ...
                    CASE 
                        ...
                        THEN julianday('now') - julianday(MAX(CASE WHEN la.status = 'success' THEN la.attempted_at END))
                        ELSE NULL
                    ...
                    ROUND(
                        (COUNT(CASE 
                            WHEN la.status = 'success' 
                            AND la.attempted_at >= date('now', '-7 days')
                            THEN 1 
                        END) * 10) +
                        (COUNT(CASE 
                            WHEN la.status = 'success' 
                        ...
                        (COUNT(CASE 
                            WHEN la.status = 'success' 
                            AND la.attempted_at >= date('now', '-90 days')
                            AND la.attempted_at < date('now', '-30 days')
                            THEN 1 
                        END) * 2), 2
                    ) as activity_score,
